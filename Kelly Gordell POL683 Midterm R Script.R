...
rm(list=ls())
setwd("C:/Users/Kelly/Desktop/683 Midterm Files/Final Midterm Files")
...
POL2 <- read.dta("national12-1.dta")
...
##################################################################
## Regression with Political Party Divide Scores
## Average percentage point difference between Republicans and Democrats
## across 48 values questions - voter differences, not congressional members


politicaldivide <- lm(weightedprediction~politicaldividescore, data=POL2)

politicaldivide

summary(politicaldivide)

stargazer(politicaldivide, type="text")


## Inputting political divide score for present to get Dem share 2 party vote

Demshare <- 0.397734 + 0.008057*(18)

Demshare


## Plot of weightedpredictions with politicaldividescore


plotdivide <- plot_ly(data=POL2, x=POL2$politicaldividescore, y=POL2$weightedprediction, name='', type = 'scatter', mode='markers')%>%
  add_trace(x=POL2$politicaldividescore, y=POL2$actualresults, name='', mode='markers')
